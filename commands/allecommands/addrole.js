...
module.exports = {
    ...
    run : async(client, message, args) => {
        ...
        if(!target) return message.channel.send('Geen persoon aangegeven') //when no member is pinged
        const role = message.mentions.roles.first() // roles = mentions
        if(!role) return message.channel.send('Geef rol aangegeven') //when no role is specified or pinged
        //now the code!
